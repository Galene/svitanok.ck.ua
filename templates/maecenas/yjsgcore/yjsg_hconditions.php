...
<?php if($topmenu_off == 2 || $itemid == 0 ) {?>
		...
		<?php } ?>
<?php if ( $default_menu_style == 3 ) {$document->addScript($yj_site.'/src/donly.js');}?>
<?php } ?>
...
<?php if($topmenu_off == 2 || $itemid == 0 ) {?>
	<?php if ($text_direction == 1) { ?>
		<style type="text/css">
		a.sublevel {
		background: url(<?php echo $yj_site ?>/images/<?php echo $css_file ?>/bodyli_rtl.png) no-repeat 98% 9px;
		}
		body li{
		background: url(<?php echo $yj_site ?>/images/<?php echo $css_file ?>/bodyli_rtl.png) no-repeat right 6px;
		}
		...
		background-image:url(<?php echo $yj_site ?>/images/<?php echo $css_file ?>/topmenu/arrow_left.png);
...
<?php } ?>
<!--Border radius-->
<?php if (!preg_match("/(msie 7|msie 8)/",$who)) { ?>
	<script type="text/javascript">
...
<?php } ?>
<?php if ($text_direction == 2 && preg_match("/(msie 7|msie 8)/",$who)) { ?>
	<style type="text/css">
		.midblock_holder,.yjsg3_out,a.check_button{
			behavior: url(<?php echo $yj_site ?>/css/pie/PIE.htc);
			z-index:0!important;
...
<?php if (preg_match("/firefox/",$who)) { ?>
	<style type="text/css">
		...
		div.itemCommentsForm form input#submitCommentButton{
			padding:4x 5px 3px 5px!important;
			font-size:75%!important;
		}
    </style>
<?php } ?>
<!--If ie9-->
<?php if (preg_match("(msie 9)",$who)) { ?>
  <style type="text/css">
    ul.subul_main,.horiznav ul.subul_main.group_holder{
		filter: progid:DXImageTransform.Microsoft.gradient(enabled = false);
	}
  </style>
<?php } ?>
<!--If ie8-->
<?php if (preg_match("(msie 8)",$who)) { ?>
  <style type="text/css">
  ul.subul_main{
	 filter: prodig:DXImageTransform.Microsoft.gradient(enabled = false);
	 background:#373a3c;
  }
  </style>
<?php } ?>

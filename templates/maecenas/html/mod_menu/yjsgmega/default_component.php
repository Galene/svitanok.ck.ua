...
if($yj_item_type > 0  && $item->level > $allow_level && !empty($mod_subclass)){
   $class = 'class="yjanchor '.$first.$last.' yj_menu_module_holder '.$mod_subclass.'" ';
}elseif($yj_item_type > 0 && $item->level > $allow_level && empty($mod_subclass)){
   $class = 'class="yjanchor '.$first.$last.' yj_menu_module_holder" ';
}else{
   $class = 'class="yjanchor '.$first.$last.'" ';
}
